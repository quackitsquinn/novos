use core::{ffi::CStr, fmt::Debug, mem::transmute};
...
pub struct ElfStrings<'a> {
    data: &'a [u8],
    section_header: &'a section_header::SectionHeader,
}

impl ElfStrings<'_> {
    pub(crate) unsafe fn new<'a>(data: &'a [u8], elf: &'a Elf<'a>) -> ElfStrings<'a> {
    ...
    pub unsafe fn get_str<'a>(&self, index: usize) -> Result<&'a str, &'static str> {
        let true_off = self.section_header.sh_offset as usize + index;
        let string =
...
impl Debug for ElfStrings<'_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_tuple("ElfStrings")
